-- /* File list12_19.lua */
-- Program for Schrodinger Wave Equation with barrier
...
xb, barrier = 0.02, -1.2e4
-- Normalized Schrodinger equation to be solved
eq = function(fv,x,t,v,vp,vpp,vt) 
	if x>=0 and x<xb then b = barrier else b = 0 end
	fv[1] = j*vt[1] + vpp[1] + b*v[1] 
end
...
x = {}; nx = 2000 -- Set initial parameters
L, alf, bet = 1, 75, 100
xo = .5*L
...
tvals = {0,.02,40,40,{-.75,-.5,-.25,0,.25,.5,.75}}
s,st = pdeivbvt({eq,efl,efr},tvals,x,v,vp,vpp)
sum = sqarea(s[1],s[#s]); print('After final solution, P = ',sum)

write_data('list12_19.dat',tomagsq(s)) -- Save data as probability density
write_data('list12_19t.dat',tomagsq(st))
