-- /* File list9_8a.lua */ -- General file for data fitting, Parameter estimation 
...
ft = function(x,c) -- Define function to fit data
	return math.exp(-c[1]*x[2])/(c[2] + c[3]*x[2]) - x[1]
end
c = {.1,.01,.02}; nc = #c -- Initial approximations. End of changes for most data sets

-- Section 2. Perform data fit and print fitted parameters
del,err,nm = nlstsq({yd,xd},fw,ft,c) -- Call fiting, print max iterations 
...
require'DataFit'
fcdf = datafit({xyp[1],xyp[2]},{{0,.05,.075,.1,.15,.2,.4,.6,.8,.95,1}})
xcdf,ycdf = {},{}
for i=1,201 do xcdf[i] = (i-1)/200;ycdf[i] = fcdf(xcdf[i]) end
plot({xcdf,ycdf},{xyp[2],xyp[1]})
write_data('list9_8tmp.dat',xcdf,ycdf)
rancdf = function(xyp)
	local fcdf = datafit({xyp[1],xyp[2]},{{0,.05,.075,.1,.15,.2,.4,.6,.8,.95,1}})
	return function()
		return fcdf(math.random())
	end
end
getfenv(MCpar).gnormal = rancdf(xyp)
