-- /* File list9_10.lua */ -- General file for data fitting, Parameter estimation 
...
infile = 'Gauss3'
xd,yd = {},{}; read_data(infile..'.txt',yd,xd) -- x stored first
nd = #xd

ft = function(x,c) -- Define function to fit data
	xx = x[2]
	return  c[1]*math.exp(-(xx - c[2])^2/c[3]) + c[4]*math.exp(-(xx - c[5])^2/c[6]) + 
	c[7]*math.exp(-c[8]*xx) - x[1]
end
c = {100,115,400,80,140,400,100,.01}; nc = #c -- Initial approximations
