-- /* File list9_17.lua */ -- Transcendental diode model 
...
ft = function(x,c) -- Define function to fit data
	return  math.log(c[1]*(math.exp((x[2]-c[3]*math.exp(x[1]))/(0.0259*c[2])) - 1)) - x[1]
end

c = {1.e-9,2,1}; nc = #c -- Initial approximations. End of changes for most data sets

-- Section 2. Perform data fit and print fitted parameters
del,err,nn = nlstsq({yd,xd},fw,ft,c) -- Call fiting, print max iterations 
print('Newton iterations =',nn)
...
write_data(infile..'d.dat',xd,yd,ycalc,res,yres1,yres2,xyp,xypn)
