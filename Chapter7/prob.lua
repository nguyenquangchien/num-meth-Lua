...
median = function(x) -- Median from a sorted table
	...
	if type(x)=="sorted" then xx = x
		...
		xx = {}
		for i=1,#x do xx[i] = x[i] end
		table.sort(xx)
		setmetatable(xx,sort_mt)
	end
...
climits = function(carr,val)
	...
	if type(carr[1])=='table' then 
		...
		if type(carr)~="sorted"  then 
			car = {}
			for j=1,nv do car[j] = carr[j] end
			table.sort(car)
		else car = carr end
...
Pchisq = function(x,a)
	if x<=0 then return 0
		...
		else return Pigamma(x/2,a/2) end
	end
end
...
Atstud = function(t,v) -- Student's A function
	...
	else	return 1 - ibeta(v/(v+t^2),v/2,0.5) end
end
...
KS_test = function(data1,data2,...) -- Includes possible arguments to function
	...
	print('\nK-S test statistic: D, sqrt(N)*D = ',engr(D),engr(kstest))
...
setfenv(ran0,{jran=885937,im=1771875,ia=2416,ic=374441,mod=math.mod})
...
setfenv(ran1,{M1=259200,IA1=7141,IC1=54773,M2=134456,IA2=8121,IC2=28411,
		...
		r={},minit=0,mod=math.mod,floor=math.floor})
...
setfenv(ran2,{M=714025,IA=1366,IC=150889,ix,iy,r={},minit=0,mod=math.mod,
	floor=math.floor})
...
setfenv(ran3,{MBIG=1000000000,MSEED=161803398,MZ=0,FAC=1/1000000000,inext,inextp,
	minit=161803398,ma={},mod=math.mod,abs=math.abs})
...
lag = function(x,lag) -- Set up Lag data tables -- Default of one lag
	lag = lag or 1
	local xx,yy,n = {},{},#x
	for i=1,n-lag do xx[i],yy[i] = x[i],x[i+lag] end
	return xx,yy
end
