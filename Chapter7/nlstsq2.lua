...
newtonfc = function(f, x, c) -- Newton's method for function with C coefficient array
	...
	if ylinear~=0 then -- Bypass Newton iterations if linear function in variable
		...
		fy = f(x, c) -- f must be of the form --- return cal_val - x[1]
		x[1] = y
		...
		for i=1,NT do -- Main Newton loop
			...
			fy = f(x, c)
			x[1] = x[1] + dy
			cy = fy - f(x, c) -- Difference for derivative
