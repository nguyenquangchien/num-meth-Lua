...
nlstsqi = function(x, fw, f, c, actv, step)
	...
	for ilp=1,NMAX do -- Main loop for Newton iterations
		...
		for i=1,nc do -- Loop over C coefficients
			if actv[i]~=0 then -- An active coefficient
				--wk2[ix] = {} -- Table in work array
				b[ix],row,wk2[i] = 0.0, rowptr[ix], {}
		...
		--for i=1,nx do p[i] = x[i][k] end; newtonfc(f, p, c, new) -- Set up solution with new c
		for k=1,nd do -- Evaluate function at data points
			for i=1,nx do p[i] = x[i][k] end
			fx[k] = newtonfc(f, p, c) --, old) 
		...
		for i=1,nc do -- Loop over active coefficients
			if actv[i]~=0 then
				c1, dx = c[i], wk1[ix]
				c[i] = c1+dx
				--newtonfc(f, p, c, new) -- Set up solution for new c 
				pwk2 = wk2[ix] -- Pointer to work array
				for k=1,nd do -- Loop over data points
					for i=1,nx do p[i] = x[i][k] end
					--pwk2[k] = (newtonfc(f, p, c, old) - fx[k])/dx -- Partial derivative evaluation
					pwk2[k] = (newtonfc(f, p, c) - fx[k])/dx -- Partial derivative evaluation
				end
				c[i], ix = c1, ix+1
			end
		...
		for j=1,nc do -- Loop over active rows in matrix
			if actv[j]~=0 then
				row = rowptr[jx] -- pointer to row of matrix
				pwk2 = wk2[jx] -- pointer to row of wk2[]
				for k=1,nd do -- Loop over data points
	...
	err = 0.0 -- Accumulate square of error
	cmax = 0.0
	for k=1,nd do -- Sum over data points
		for i=1,nx do p[i] = x[i][k] end
		c1 = x[1][k] - newtonfc(f, p, c) -- Error here
		cmax = cmax + c1
...
setfenv(nlstsqi,{ERROR=5.e-4,FACT=1.e-8,NMAX=50, ERR=1.e-3,NT=50,
...
setfenv(newtonfc,getfenv(nlstsqi)) -- Same parameters for newtonfc()
